 ...
 * 빈 타일에 문자 채우기
 */
function addLetter(tbdList, letter) {
  if (tbdList.length < 5) {
...
function handleClickKeyboard(target) {
  ...
  if (letter === "remove") removeLetter(tbdList);
 ...
 * 엔터 입력 시에 동작하는 함수
 */
function handleEnterEvent(tbdList, emptyList) {
 ...
 * 모달 토글 함수
 */
function toggleModal(text, isOpen) {
  setTimeout(function () {
